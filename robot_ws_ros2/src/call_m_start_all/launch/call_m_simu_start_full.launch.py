...
def generate_launch_description():
    ...
    slam_param = os.path.join(dir_slam, 'config/mapper_params_online_async_simu.yaml') #path to params for slam
    ...
    nav2_launch_loc = launch.actions.ExecuteProcess(cmd=cmd_debug + ['call_m_nav2', 'localization_launch.py','params_file:='+nav_mode,'map:='+map_loc] + suffix, output='screen')
    ...
    if nav_type == "on_fly":
        ...
        return LaunchDescription([master_launch,simu_launch,display_launch,teleop_launch,nav2_launch_loc,nav2_launch])
    else:
